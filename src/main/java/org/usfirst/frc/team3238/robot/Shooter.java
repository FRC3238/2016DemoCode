...
public class Shooter {
     ...
     */
    Shooter(WPI_TalonSRX shooterLeftTalon, WPI_TalonSRX shooterRightTalon,
    ...
    private void setPowerOverride(double shooterPower) {
        ...
        else { //if neither than just set it as shooterpower and adjust the value based on RPM
    ...
    public boolean autoShoot(boolean shoot, boolean disable) {
        if(disable) {
            setPowerOverride(0);
            running = false;
        }
        ...
        DriverStation.reportError("" + t.get(), false);

        if(running && t.get() < 1) {

            setPowerOverride(0.78); //.78
        }
        else if(running && t.get() < 1.7) {
            setPowerOverride(0.78); //.78
